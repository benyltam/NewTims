...
namespace NewTims
{
    public partial class StudentList_Form : Form
    {
        ...
        {
            string sSql = "SELECT Top 14 Name,Format(StudentID,'000000') as [Student ID],MiddleName,ChiName,Remark "
                + "FROM Student "
                + "Where Cast(Name as char(100))+Cast(StudentID as char(10))>="
                + "'" + (s_KeyBuffer + String(" ", 100)).Substring(0, 100) + s_StudentID + "'"
        ...
        private void StudentList_dataListView_KeyPressEventHandler(object sender, System.Windows.Forms.KeyPressEventArgs e)
        {
            ...
            switch (ascII[0])
            {
                case byte m when m > 32 && m < 255:
                    ...
                    {
                        //var m_Name = StudentList_dataListView.Items(i_LastRow - 1).SubItems[1].Text.Trim();
                        s_KeyBuffer = s_KeyBuffer.Trim() + e.KeyChar.ToString();
                        if (s_Name.Substring(0,s_KeyBuffer.Length) != s_KeyBuffer)
                        {
        ...
        private void SetColumnWidth()
        {
            ...
            StudentList_dataListView.Columns[4].Width = 210;
        }
        ...
        private void Next_button_StudentList_Click(object sender, EventArgs e)
        {
            ...
            s_KeyBuffer= (s_Name + String(" ", 100)).Substring(0, 100) + s_StudentID;
            FillDataListView();
            s_KeyBuffer = "";
        }
